...
async function updateFinTotalsAndDates(finId: string, tx: any) {
  ...
  if (allBits.length > 0) {
    ...
    const calculatedTotal = allBits.reduce(
      (acc: number, bit: { amount: string | null; date: Date }) =>
        acc + parseFloat(bit.amount || "0"),
      0
    );
    totalAmount = calculatedTotal.toFixed(2); // Format to 2 decimal places

    // Find min and max dates
    const dates = allBits.map((bit: { amount: string | null; date: Date }) =>
      bit.date.getTime()
    ); // Get timestamps
...
export default defineEventHandler(async (event) => {
  try {
    // Outer try for the whole handler
    ...
    return await db.transaction(async (tx) => {
      return await withSession(request, async (err, session) => {
        if (err) {
          ...
          throw createError({
            statusCode: 500,
            statusMessage: err.message || "Session error",
          });
        }
        ...
        const userEmail = await getUserDetails(supertokensId).then(
        ...
        if (!finId) {
          throw createError({
            statusCode: 400,
            statusMessage: "Fin ID is required",
          });
        }
        ...
        if (finCheck.length === 0) {
          throw createError({
            statusCode: 403,
            statusMessage: "Unauthorized access to this fin",
          });
        }
        ...
        if (event.method === "POST" || event.method === "PUT") {
          ...
          if (event.method === "PUT") {
            ...
            if (!body.id) {
              throw createError({
                statusCode: 400,
                statusMessage: "Missing bit ID for update",
              });
            }
            const updateResult = await tx
              ...
              .where(
                and(eq(bitsTable.id, body.id), eq(bitsTable.fin_id, finId))
              )
            ...
            if (updateResult.length === 0) {
              throw createError({
                statusCode: 404,
                statusMessage: "Bit not found or update failed",
              });
            }
            ...
            if (insertResult.length === 0) {
              throw createError({
                statusCode: 500,
                statusMessage: "Failed to create bit",
              });
            }
            ...
            return new Response(
              JSON.stringify({ message: "success", id: insertResult[0].id }),
              {
                status: 201,
                headers: { "Content-Type": "application/json" },
              }
          ...
          if (!body.id) {
            throw createError({
              statusCode: 400,
              statusMessage: "Missing bit ID for delete",
            });
          }
          ...
          if (deleteResult.length === 0) {
            throw createError({
              statusCode: 404,
              statusMessage: "Bit not found or unauthorized",
            });
          }
